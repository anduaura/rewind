...
| E4 | **Service attribution in Kubernetes** — `ServiceMap` uses `docker inspect` which doesn't work in Kubernetes pods. Need cgroup v2 path parsing for k8s (`/sys/fs/cgroup/kubepods/...`) and Pod → service name resolution via the Downward API or the kubelet API. | Capture agent | Done | `parse_container_id` extended for containerd (`cri-containerd-`) and CRI-O (`crio-`) cgroup paths + k8s cgroup v1 kubepods paths; `crictl ps` populates container→service map from pod labels at startup; `/proc/<pid>/environ` HOSTNAME fallback with `strip_pod_suffix` heuristic for pods not in static map. |
| E5 | **No load / scale validation** — collection server has never served production traffic; no load test, no chaos test, no proven shared-storage backend under concurrent writes. | Collection server | Done | `rewind/src/bin/load_test.rs` — async Rust binary: N VU upload+list+download loop, p50/p90/p95/p99 latency, error-rate threshold, exits 1 on failure. `tests/load/k6-server.js` — k6 script with smoke/load/stress/spike scenarios and thresholds. Two new integration tests: `concurrent_uploads_all_succeed` (20 threads, all succeed) + `rate_limit_triggers_429`. `make load-test` / `make load-test-k6` targets. |
...
| E7 | **SaaS web UI is missing** — milestone 35 is marked Done but no browser-based UI exists; the collection server has API endpoints but no snapshot browsing, timeline visualisation, or team management UI. | Developer experience | Done | `rewind server` now serves `/ui` (dashboard with filter + stats) and `/ui/:name` (event table, summary cards, inline Mermaid sequence diagram, share + delete). `DELETE /snapshots/:name` API added. `Backend::delete()` implemented for local + object-store backends. Self-contained HTML/CSS/JS, dark theme, no build step. |
